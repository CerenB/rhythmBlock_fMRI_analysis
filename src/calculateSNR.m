...
else strcmpi(opt.taskName, 'RhythmFT') 
    if opt.nStepsPerPeriod==4
        opt.whichHarmonics = [1,2];  
    end
end

opt.derivativesDir = fullfile(paths.deriv, 'cpp_spm');
...
fileName = [getOutputFileName(valueName,opt), '.fig'];
saveas(f, fileName); 
close(f); 
...
% ----------------------------------------------------------------
% amplitudes at target frequency after SNR baseline subtraction 
blfun = @(x,y) x-y; 
mXSNRAmp = baselineCorrect(abs(FT), cfg, 'fun', blfun); 
targretAmp = mXSNRAmp(cfg.targetFrequency,:); 

% plot best voxels 
f = plotmXBestVox(freq, mXSNRAmp, targretAmp, 10, cfg.idxHarmonics); 

% save figure 
valueName = 'AvgAmpTarget-bestVox'; 
fileName = [getOutputFileName(valueName,opt), '.fig'];
saveas(f, fileName); 
close(f); 

% save map as nii
valueName = 'AvgAmpTarget'; 
writeMap(targretAmp, maskFileName, valueName, opt)
...
function writeMap(data2write, maskFileName, valueName, opt)
    ...
    fileName = [getOutputFileName(valueName,opt), '.nii'];
...
function fileName = getOutputFileName(valueName, opt)

    fileName = fullfile(opt.destinationDir, [valueName, ...
                                             '_sub-', opt.subject{1}, ...
                                             '_ses-', opt.session{1}, ...
                                             '_task-', opt.taskName, ...
                                             '_bold']);
