function bidsRemoveDummies(opt)
    ...
    if ~isfield(opt, 'numDummies')
        ...
        error(['Number of dummies is not specified!!'...
               ' Leave opt.numDummies empty if you just want to unzip files.']);
        ...
        for iGroup = 1:length(group)
            ...
            for iSub = 1:group(iGroup).numSub
                ...
                % Go to directory to run getSessions & GetRuns functions
                % cd(WD);
                % Get the subject ID
                ...
                for iSes = 1:numSessions    % for each session
                    ...
                    for iRun = 1:numRuns
                        ...
                        if numDummies <= 0 || isempty(numDummies)
                            % Save the functional data as unzipped nii
                            save_untouch_nii(n, fileName(1:end - 4));
                            ...
                            nDummies = n;
                            nDummies.img = nDummies.img( ...
                                                        :, :, :, 1:numDummies);
                            % Change the dimension in the header
                            nDummies.hdr.dime.dim(5) = size(nDummies.img, 4);
                            save_untouch_nii( ...
                                             nDummies, fullfile(dummiesOuputDir, ...
                                                                ['dummies_', fileName]));

                            % Create the 4D functional files without the dummies and
                            % save them
                            nNoDummies = n;
                            nNoDummies.img = nNoDummies.img( ...
                                                            :, :, :, numDummies + 1:end);
                            % Change the dimension in the header
                            nNoDummies.hdr.dime.dim(5) = size( ...
                                                              nNoDummies.img, 4);
                            % dr : dummies removed
                            save_untouch_nii( ...
                                             nNoDummies, [prefix, fileName(1:end - 3)]);
